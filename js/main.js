window.addEventListener("load", () => {
  ...
  const displayItems = (items) => {
    for (const item of items) {
      if (item.links && item.links.length > 0) {
        const imgSrc = item.links[0].href !== null ? item.links[0].href : "https://mf600.brighton.domains/CI527/assignment/one/imgs/no_image.png";
  ...
  const createImageElement = (imgSrc, imgAlt, imgTitle, imgDesc, imgDate) => {
    ...
    img.addEventListener("error", () => {
      img.src = "https://mf600.brighton.domains/CI527/assignment/one/imgs/no_image.png";
      img.alt = "Image error";
